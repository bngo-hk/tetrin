...
//ミノ種類定数定義
  ...
  const bgm_button = document.querySelector('.button_bgm')
  const se_button = document.querySelector('.button_se')
  const credit_button = document.querySelector('.button_credit')
  //ページ要素取得
  const pg_top = document.querySelector('.pg_top')
  const pg_howto = document.querySelector('.pg_howto')
  const pg_credit = document.querySelector('.pg_credit')
  ...
  const oni_credit = document.querySelector('.credit_oni')
  //テーブル要素取得
  ...
  //効果音
  const se_hold = new Audio('./sounds/hold.mp3');
  const se_move = new Audio('./sounds/move.mp3');
  const se_spin = new Audio('./sounds/spin.mp3');
  const se_hardDrop = new Audio('./sounds/harddrop.mp3');
  const se_oni = new Audio('./sounds/oniactive.mp3');
  const se_buttonPush = new Audio('./sounds/push_button.mp3')
  const se_delete = new Audio('./sounds/delete.mp3')
  const se_gameOver = new Audio('./sounds/gameOver.mp3')
  const se_clear = new Audio('./sounds/clear.mp3')
  const se_shara = new Audio('./sounds/shara.mp3')
  const se_miss = new Audio('./sounds/miss.mp3')
  const se_stop = new Audio('./sounds/stop.mp3')
  const se_levelup = new Audio('./sounds/levelup.mp3')
  const se_score = new Audio('./sounds/score.mp3')

  const bgm_main = new Audio('./sounds/bgm_main.mp3')
  const bgm_oni = new Audio('./sounds/bgm_oni.mp3')
  bgm_main.load()
  bgm_oni.load()
  let bgm_main_loop = {}
  let bgm_oni_loop = {}
  ...
  credit_button.addEventListener("click", ()=>{
      pg_credit.style.display="block"
      pg_credit.classList.add("pg_credit_active")
    //ボタン音
    se_buttonPush.currentTime=0
    se_buttonPush.play()
  })
  pg_credit.addEventListener("click", ()=>{
      pg_credit.classList.remove("pg_credit_active")
      pg_credit.style.display="none"
    //ボタン音
    se_buttonPush.currentTime=0
    se_buttonPush.play()
  })
  ...
  bgm_button.addEventListener("click", ()=>{

      if(bgm_button.classList[1]==="bgmOn"){
        bgm_button.classList.remove("bgmOn")
        bgm_button.classList.add("bgmOff")
        bgm_button.innerHTML="BGM<br/>OFF"

        bgmVolume(0)
      }
      else{
        bgm_button.classList.remove("bgmOff")
        bgm_button.classList.add("bgmOn")
        bgm_button.innerHTML="BGM<br/>ON"
  ...
  se_button.addEventListener("click", ()=>{
      if(se_button.classList[1]==="seOn"){
        se_button.classList.remove("seOn")
        se_button.classList.add("seOff")
        se_button.innerHTML="SE<br/>OFF"

        seVolume(0)
      }
      else{
        se_button.classList.remove("seOff")
        se_button.classList.add("seOn")
        se_button.innerHTML="SE<br/>ON"
  ...
  //ボリューム設定
  function bgmVolume(volume){
    bgm_main.volume=volume * 0.5;
    bgm_oni.volume=volume * 0.5;
  }
  function seVolume(volume){
    se_hold.volume = volume * 1
    se_move.volume = volume * 1
    se_spin.volume = volume * 0.8    
    se_hardDrop.volume = volume * 1
    se_oni.volume = volume * 1
    se_buttonPush.volume = volume * 0.6
    se_delete.volume = volume * 0.8
    se_gameOver.volume = volume * 0.6
    se_clear.volume = volume * 0.2
    se_shara.volume = volume * 1
    se_miss.volume = volume * 0.5
    se_stop.volume = volume * 0.5
    se_levelup.volume = volume * 1
    se_score.volume = volume * 1
  }
  ...
  function setkeydown(){
    ...
    else if(e.key==='k')
    {
      ...
      else if(oniCnt===3)
      {
        ...
        oni_credit.style.display='block';
        se_oni.play()
      }
      else
      {
...
class Manager{
  ...
  async deleteLine(deleteLineList,tspin){
    ...
    //効果音
    se_hardDrop.pause()
    se_hardDrop.currentTime=0
    se_delete.pause();
    se_delete.currentTime=0
    se_delete.play();
    this.updateField([])
  }
  //レベル管理
  ...
  gameOverEf(){
      ...
      se_miss.pause()
      se_miss.currentTime=0
      se_miss.play()
      setTimeout(()=>{se_gameOver.play()},1200)
  }

  gameOver(){
    //bgm
    if(oniFlg){
      bgm_stop(bgm_oni,bgm_oni_loop)
    }
    else{
      bgm_stop(bgm_main,bgm_main_loop)
    }
...
//ミノ生成関数
  ...
  function createTweet(time,score,rank){
    ...
    const url='http://twitter.com/share?text='+ msg +'&url=https://bngo-hk.web.app&lang=Japanese&count=none&hashtags=TETRIN';
  ...
  class bgm_loop{
    ...
    mainLoop(bgm,bgmLooptime,bgmStartTime,bgmResetTime){
      if(!this.continueFlg)
        {
          return false
        }
      bgm.pause()
      if(this.startFlg){
        bgm.currentTime=bgmStartTime;
        this.startFlg=false
        bgm.play();
        setTimeout(()=>{this.mainLoop(bgm,bgmLooptime,bgmStartTime,bgmResetTime)}, (bgmLooptime-bgmStartTime)*1000)
      }
      else{
        bgm.currentTime=bgmResetTime;
        bgm.play();
        setTimeout(()=>{this.mainLoop(bgm,bgmLooptime,bgmStartTime,bgmResetTime)}, (bgmLooptime-bgmResetTime)*1000)
      }
    }
  }
  //bgm停止
  function bgm_stop(bgm,bgm_loop_obj) { 
    bgm.pause();
    bgm_loop_obj.continueFlg=false;
    bgm_loop_obj="";
  }
...
async function controller(){
    //スリープ処理
    await sleep(30000/60)
    //メインBGM
    if(oniFlg){
      bgm_oni_loop = new bgm_loop()
      bgm_oni_loop.mainLoop(bgm_oni,327.57,0,11.33)
    }
    else{
      bgm_main_loop = new bgm_loop()
      bgm_main_loop.mainLoop(bgm_main,44.23,0,0)
    }
    ...
    //---------------------
      ...
      window.onkeydown = (e) => {
        fallTF = mino.fallCheck();
         if(!fallTF && touchedFrame > Math.floor(30 - level/1.5 + oniMargin))
         {
           allowKey=false;
           minoResetFlg=true
         }
...
//描画ループ開始
  ...
  {  
    ...
    while(minoResetFlg)
    {
      ...
      if(frame-startFrame > Math.floor(59/(level + oniMargin )))
      { 
        //猶予フレーム計測
        if(touchedStart)
        {
    ...
    if(line>=150){
      ...
      await sleep(640)
      se_shara.pause()
      se_shara.currentTime=0
      se_shara.play()
      await sleep(500)
      for(let i=0;i<3;i++){
        await sleep(1750)
        se_score.pause()
        se_score.currentTime=0
        se_score.play()
      }
